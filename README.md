...
In AWS Device Farm Remote Access, upload/install the APK, launch `QPNPU Hardware Probe`, confirm the hosted manifest URL is present, and tap `Shard Load`.

The app logs Phase 9 JSON between:
...
Phase 9 validates verified cached shard delivery into native file loading only. The native code opens metadata and shard paths itself, but the artifact is still a tiny toy model and does not use Qwen 9B, QNN, NPU, or a real performance benchmark.

## Phase 8 External Toy Model Delivery Quickstart

Create the tiny external artifact locally:

```bash
python scripts/model/create_external_toy_artifact.py --out models/external_toy_qwen_smoke --hidden-size 16 --overwrite
```
...
In AWS Device Farm Remote Access, upload/install the APK, launch `QPNPU Hardware Probe`, and tap `External Model`. Leave the manifest URL blank for the bundled fallback demo, or paste this hosted Hugging Face test manifest URL:

```text
https://huggingface.co/yingjieh512/toy-model-for-testing/resolve/main/external_toy_qwen_smoke/manifest.json
```

The app logs Phase 8 JSON between:
...
Phase 8 validates external-style model delivery, cache, checksum, and toy decode plumbing only. It is not real Qwen 9B inference and does not use or measure the phone NPU.


## Phase 7C Generated Kernel Candidate Quickstart
...
In AWS Device Farm Remote Access, upload/install the APK, launch `QPNPU Hardware Probe`, and tap `Gen Kernels`. The app logs generated-kernel candidate JSON between:
...
Phase 7C validates generated native CPU candidate plumbing and correctness only. SVE2, SVEI8MM, and SME candidates are listed but deferred until safer isolation exists. Do not treat any latency field as a trial performance claim.
...
- A minimal Android probe app project exists under `android/probe-app` with an NDK/JNI CPU microbenchmark harness, generated CPU kernel candidates, packaged toy decode assets, external toy model delivery, and a native cached-shard loader smoke path.
...
- The Android probe APK does not run real Qwen inference, QNN, or NPU code; Phase 5 native microbenchmarks, Phase 7B toy decode, Phase 7C generated candidates, Phase 8 external toy delivery, and Phase 9 native shard loading are tiny CPU-only harness checks.
...
- No Qwen 9B, accelerator, or NPU benchmark exists; Phase 5, Phase 7B, and Phase 7C only validate native CPU execution, model-asset plumbing, and generated-candidate correctness plumbing.
